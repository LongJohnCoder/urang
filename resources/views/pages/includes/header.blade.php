...
$(function() {
	var timer;
    var divpos = $('.sticky-text').offset().top;
    $(window).scroll(function() {
    	 if ( timer ) clearTimeout(timer);

        var scroll = $(window).scrollTop();

        var newPos = parseInt(divpos + scroll);

         timer = setTimeout(function(){ $('.sticky-text').css({
        	"top": newPos,
        	"position":"absolute"
        });
     }, 300);
...
<script>
	$(document).ready(function() {
		...
		$(window).scroll(function(){			
			$scrollingDiv
				.stop()
				.animate({"top": ($(window).scrollTop() + 270) + "px"
			}, 300 );
